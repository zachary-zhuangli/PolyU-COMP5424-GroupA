...
#pragma warning restore 162
        ...
        public static void DrawTerrainDetailPrototype(ResourceProtoDetail resourceProtoDetail, EditorUtils editorUtils, Spawner spawner, bool showHelp)
        {
            ...
            if (GUILayout.Button(editorUtils.GetContent("DetailsButtonDisablePWGrassAll")))
            {
                if (EditorUtility.DisplayDialog("Disable All Flora Spawn Rules?", "Do you want to disable the Flora setting for all spawn rules in this spawner?", "Yes, Disable All", "Cancel"))
                {
                    disabledByButton = true;

                    bool completeRemoval = false;
                    if (EditorUtility.DisplayDialog("Remove from Terrain as well?", "Do you also want to remove any already spawned items from the Flora configuration on the terrain? These detail items will then not be rendered by Flora anymore during runtime.", "Remove from Terrain", "Keep On Terrain"))
                    {
                        completeRemoval = true;
                    }
            ...
            if (resourceProtoDetail.m_useFlora)
            {
                ...
                //Mesh / Material
                editorUtils.LabelField("DetailProtoHeadingPWGrassMesh", EditorStyles.boldLabel);
                editorUtils.ObjectField("DetailPWGrassMesh", null, typeof(Mesh), false, showHelp);
                editorUtils.ObjectField("DetailPWGrassMaterial", null, typeof(Material), false, showHelp);
                //Color
                editorUtils.LabelField("DetailProtoHeadingPWGrassColor", EditorStyles.boldLabel);
                editorUtils.ColorField("DetailPWGrassColorA", Color.gray, showHelp);
                editorUtils.ColorField("DetailPWGrassColorB", Color.gray, showHelp);
                editorUtils.CurveField("DetailPWGrassColorTransition", new AnimationCurve(), showHelp);
                editorUtils.Slider("DetailPWGrassColorRandomization", 50f, 0f, 100f, showHelp);

                //Drawing
                editorUtils.LabelField("DetailProtoHeadingPWGrassDraw", EditorStyles.boldLabel);
                editorUtils.Toggle("DetailPWGrassDisableDraw", false, showHelp);
                editorUtils.IntField("DetailPWGrassDrawBias", 1, showHelp);
                editorUtils.FloatField("DetailPWGrassStartFade", 10, showHelp);
                editorUtils.FloatField("DetailPWGrassEndFade", 50, showHelp);
                editorUtils.EnumPopup("DetailPWGrassShadowMode", ShadowCastingMode.TwoSided, showHelp);

                //Position
                editorUtils.LabelField("DetailProtoHeadingPWGrassPosition", EditorStyles.boldLabel);
                editorUtils.IntSlider("DetailPWGrassDensity", 50, 2, 128, showHelp);
                editorUtils.Slider("DetailPWGrassMaxJitter", 75f, 0, 100, showHelp);

                //Scale
                editorUtils.LabelField("DetailProtoHeadingPWGrassScale", EditorStyles.boldLabel);
                float scaleRangeMin = 2;
                float scaleRangeMax = 4;
                editorUtils.MinMaxSliderWithFields("DetailPWGrassScaleRange", ref scaleRangeMin, ref scaleRangeMax, 0, 16f, showHelp);

                editorUtils.CurveField("DetailPWGrassScaleByAlpha", new AnimationCurve(), showHelp);

                //Rotation
                editorUtils.LabelField("DetailProtoHeadingPWGrassRotation", EditorStyles.boldLabel);
                editorUtils.Slider("DetailPWGrassForwardAngleBias", 0, -180, 180, showHelp);
                editorUtils.Slider("DetailPWGrassRandomRotationRange", 100f, 0, 100, showHelp);
                editorUtils.Toggle("DetailPWGrassUseAdvancedRotations", false, showHelp);

                //Threshold Mask
                editorUtils.LabelField("DetailProtoHeadingPWGrassAlphaThreshold", EditorStyles.boldLabel);
                editorUtils.Slider("DetailPWGrassAlphaThreshold", 75f, 0, 100, showHelp);

                //Noise Mask
                editorUtils.LabelField("DetailProtoHeadingPWGrassNoiseMask", EditorStyles.boldLabel);
                editorUtils.Toggle("DetailPWGrassUseNoise", true, showHelp);
                editorUtils.Slider("DetailPWGrassNoiseScale", 50, 0, 100, showHelp);
                editorUtils.Slider("DetailPWGrassNoiseContrast", 2, 0, 8, showHelp);
                editorUtils.Slider("DetailPWGrassNoiseOffset", 0, -4, 4, showHelp);
                editorUtils.Toggle("DetailPWGrassInvertNoise", false, showHelp);

                //Debug
                editorUtils.LabelField("DetailProtoHeadingPWGrassDebug", EditorStyles.boldLabel);
                editorUtils.Toggle("DetailPWGrassDrawCPUSpawnLocations", false, showHelp);
                editorUtils.ColorField("DetailPWGrassDebugColor", Color.gray, showHelp);
